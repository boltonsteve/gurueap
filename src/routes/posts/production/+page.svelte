<script>
    import SvelteSeo from "svelte-seo"
    import WithIcon from '../../components/WithIcon.svelte'
</script>

<SvelteSeo
  title="Production(s)"
  description="Examining the different meanings of procuction countable and production uncountable"
  keywords="IELTS,EAP,academic English,Indonesia,study abroad,English for academic purposes,pre-departure training,vocabulary,countable uncountable" 
/>

<div class="mb-3 mw-500">

    <img src="/img/production.jpg" class="img-fluid rounded mt-1" alt="production">

    <h1>Production(s)</h1>

    <div class="alert alert-danger font-italic">Automation in industry means increased productivity and better <span class="font-weight-bold text-danger">productions</span>.</div>

    <WithIcon icon="warning" msg="Yet another word that has quite different meanings in its countable and uncountable forms!"/>

    <h3 class="mb-0">Production (countable)</h3>
    <p class="mt-0">In its countable form, <a href="https://skell.sketchengine.eu/#result?lpos=&query=productions&lang=en&f=concordance" target="_blank" rel="noopener"><em><strong>production</strong></em></a> has strong associations with music and theatre:</p>

 	<ul class="lh-15">
 	    <li>Medieval theatre <b><i>productions</i></b> are still performed today.</li>
 	    <li>300 performances were given of 33 different opera <b><i>productions</i></b>.</li>
 	    <li>The earliest sound effects were strictly studio <b><i>productions</i></b>.</li>
 	    <li>Previous acclaimed <b><i>productions</i></b> include "Oklahoma!"</li>
 	    <li>The building was used for massive concerts and theatrical <b><i>productions</i></b>.</li>
 	</ul>

    <h3 class="mb-0">Production (uncountable)</h3>
    <p class="mt-0">It is only when it is in its <b><i>uncountable</i></b> form that production means <b><i>manufacturing</i></b>:</p>

 	<ul class="lh-15">
 	    <li>The highest <b><i>production</i></b> recorded was fifty thousand annually.</li>
 	    <li>The company has 15 <b><i>production</i></b> plants worldwide.</li>
 	    <li>By 1900 daily <b><i>production</i></b> was 2 thousand tons.</li>
 	    <li>Even small scale 'capitalist' <b><i>production</i></b> was suppressed.</li>
 	    <li>The pellets <b><i>production</i></b> required increased freshwater access.</li>
 	</ul>

    <p>And so returning to our opening example, we need:</p>

    <div class="alert alert-success font-italic mb-2">Automation in industry means increased <span class="font-weight-bold text-success">productivity</span> and better <span class="font-weight-bold text-success">production</span>.</div>

    <WithIcon icon="sign" position="right">
        <div slot="html">Elsewhere on GuruEAP you will find <a href="../posts/countable-uncountable">further discussion and examples of countable and uncountable nouns</a>.</div>
    </WithIcon>

</div>
